...
const ConfirmOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const confirmedOrders = (data.orders || []).filter(
        (order: any) => order.orderStatus === "confirmed"
      );
  ...
  const downloadInvoice = async (order: any) => {
    ...
    doc.setFontSize(10);
    doc.text(
      `Customer: ${order.buyerId?.firstName || ""} ${order.buyerId?.lastName || ""}`,
      14,
      61
    );
    doc.text(`Email: ${order.buyerId?.email || "N/A"}`, 14, 68);
    doc.text(`Location: ${order.location || "N/A"}`, 14, 75);
    doc.text(`Completed Date: ${orderDate}`, 14, 82);
    ...
    doc.setFontSize(15);
    doc.text("Payment Receipt", 14, y + 40);

    if (order.paymentReceipt) {
      const receiptBase64 = await imageToBase64(order.paymentReceipt);

      if (receiptBase64) {
        doc.addImage(receiptBase64, "JPEG", 14, y + 47, 70, 70);
      } else {
        doc.setFontSize(10);
      ...
      <div className="mb-8 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className={`${bodyFont} mt-3 text-[14px] text-[#6f6a63]`}>
            Successfully completed and confirmed customer orders.
          </p>
        </div>
